...
def main():
    ...
    for i in range(ROUNDS):
        while not battle(player_list, k):
            pass
    ...
    '''
    strength_list = [p.strength for p in player_list]
    empi_elo_list = [p.get_average_elo(0.1) for p in player_list]
    theo_elo_list = [p.theoretical_mean for p in player_list]
    empi_scatter = plt.scatter(strength_list, empi_elo_list)

    theo_scatter = plt.scatter(strength_list, theo_elo_list)
    plt.legend((empi_scatter, theo_scatter), ('Empirical', 'Theoretical'))
    plt.xlabel('Strength')
    plt.ylabel('Elo')
    
    empi_linear_reg = np.polyfit(strength_list, empi_elo_list,1 )
    theo_linear_reg = np.polyfit(strength_list, theo_elo_list,1 )
    print('empi linear reg s = {}, b = {}'\
            .format(empi_linear_reg[0], empi_linear_reg[1]))
    print('theo linear reg s = {}, b = {}'\
            .format(theo_linear_reg[0], theo_linear_reg[1]))
